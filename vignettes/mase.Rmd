...
$$
where the errors $\epsilon_i$ would structurally have variance $\sigma ^2 x_i$. 
...
When $y$ is quantitative, GREG is a linear regression model as follows:
$$
y_{i} = \beta_{o} + \beta_{1}x_{i1} + \beta_{2}x_{i2} + ... + \beta_{p}x_{ip} + 
        \epsilon_{i} \\
      = \boldsymbol{x}_{i}^T \boldsymbol{\beta} + \epsilon_{i}
$$
The following code calculates GREG for our quantitative $y$ variable, api, in our srs and strat sample datasets using the mase package:

When $y$ is quantitative, GREG is a linear regression. The following code calculates GREG for our quantitative $y$ variable, api, in our srs and strat sample datasets using the mase package:
```{r}
...
When $y$ is categorical, GREG is a logistic regression model as follows:
$$
P(y_{i} = 1|\boldsymbol{x}_{i}) = \frac{exp(\boldsymbol{x}_{i}^T \boldsymbol{\beta})}
                                  {1+exp(\boldsymbol{x}_{i}^T \boldsymbol{\beta})}
$$
The logistic model results in probabilities between 0 and 1. When $y$ is categorical, GREG is a logistic regression. The following code calculates GREG for our categorical $y$ variable, awards, in our srs and strat sample datasets using the mase package:
```{r}
# GREG applied to simple random sample
...
The Elastic Net Regression estimator is simply the same as the GREG estimator above, with an added condition in the original formula which uses a penalized survey-weighted least squares to incorporate model selection when estimating our coefficients. This added condition is called the "elastic net." 
$$
\hat{\boldsymbol{\beta}_{s}} = \underset{\boldsymbol{\beta}}{\text{arg min}} 
                              ...
                              \right \}
$$
first term = lasso penalty
second term = ridge regression penalty
alpha = how much lasso penalty vs ridge penalty
lasso is good for shrinking unnecessary variables
ridge is good for multicollinearity
better than brute force choosing subsets because too tedious. penalization section chooses the "important" betas for us

With the addition of the elastic net condition, the Elastic Net Regression estimator has a better variability than the GREG. The presence of too many variables from available auxiliary data can add extra variability to GREG, whereas the Elastic Net estimator adds a penalization which shrinks the coefficients towards zero. 

Likewise with GREG, one can calculate linear and logistic regressions using the Elastic Net estimator. The following code calculates the Elastic Net for our quantitative $y$ variable, api, in our srs and strat sample datasets using the mase package.
```{r}
# Elastic Net applied to simple random sample
...
# Elastic Net applied to stratified sample
...
```
make comment on how std hopefully shrinks
