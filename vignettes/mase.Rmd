...
In particular, we note that the quantity $\frac{1}{\hat{N}_j} \sum _{i \in S_j} \frac{y_i}{\pi_i}$ 
#is what?
...
The above gives the estimated mean,`r ps_srs_r$pop_mean`, assuming simple random sampling. 
#Do I need to discuss this result a bit more? 
...
$$
to which we give the name ratio estimator. @sar92. We can see that this estimator builds on the Horvitz Thompson estimator because it addresses the Horvitz Thompson estimator of $\mu_y$ by 
...
-Bootsrap
#How should I talk about bootstrap? 


#Hansen-Hurwitz
The Hansen-Hurwitz estimator is an estimator of $\frac{y_i}{\pi_i}$. The difference between the Horvitz-Thompson estimator and the hansen-hurwitz estimator is that the HT(SRS?) is used for sampling without replacement and HH for sampling with replacement. (But doesn't HT estimator work for any design, with or wihout replacement?) The expression is the same as the one for the Horvitz Thompson Estimator.  
$$
\hat{\mu}_{HH} = \frac{1}{n} \sum_{i=1}^n \frac{y_i}{pi}
$$
The convenience of HH estimator is that it is easier to compute, but with the drawback that it has a higher variance. Recalling that $t$ stands for the population sum, we have that the variance for this estimation is 
$$
Var(\hat{\mu}_{HH}) = \frac{1}{n}\sum_{i=1}^n \pi_i (\frac{y_i}{\pi_i} - t) ,
$$
and an (unbiased) estimator that on average gives the expected value of this variance is 
$$
\hat{Var}(\hat{\mu}_{HH}) = \frac 1 n \frac{\sum _{i=1} ^ n (\frac{y_i}{\pi} - \hat{t})^2}{n-1}.
#I guess it would be good to know how this estimator should ideally be introduced
#Also note that this estimator is different from the one introduced on pg 121. 
$$

#Horvitz-Thompson Estimator and its variance (general and SRS)
 It can be shown that the variance of the Horvitz Thompson estimator is
 $$
 Var(\hat{t}_{ij?}) = \sum _{i = 1} ^ N (\frac{1-\pi_i}{\pi_i}) y_i^2 + \sum _{i = 1} ^N \sum _{j\neq i} (\frac{\pi_{ij} - \pi_i\pi_j}{\pi_i \pi_j}) y_i y_j,
 $$
 so the logical estimate is
  $$
 Var(\hat{t}_{ij?}) = \sum _{i = 1} ^ n (\frac{1-\pi_i}{\pi_i}) y_i^2 + \sum _{i = 1} ^n \sum _{j\neq i} (\frac{\pi_{ij} - \pi_i\pi_j}{\pi_i \pi_j}) y_i y_j.
 $$
#Should I raise an example also related to api? 
### References