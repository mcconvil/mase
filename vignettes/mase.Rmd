...
We will note that, of the estimators we will introduce in this documentation, Horvitz Thompson estimators does not make use of auxilary data, and with the exception of the logistic greg estimators (including the one with elastic net) our estimators are compatiable with all three forms--raw, with total counts and population size, and with mean counts and population size.
...
An overview of the various variance estimators is given at the last section of this documentaiton. 
...
It is not uncommon for us to have categorical auxilary data that might give information we would like to use in modeling. It has been shown that such information can often increase the estimator's efficiency in that it could increase the precision of our estimates by reducing variance. For instance, that a particular location is covered by forest would give good predictor of canopy cover since the plots of land labelled "forest"" is much more likely to be covered by canopy than those that are labelled "non-forest".
...
re_srs_raw <- ratioEstimator(y = apisrs$api00, xsample = apisrs$meals, xpop = apipop$meals, datatype = "raw", pi = NULL, N = NULL, pi2= NULL, var_est = TRUE, var_method = "HB", B = 1000)
re_srs_raw
...
re_srs_totals <- ratioEstimator(y = apisrs$api00, xsample = apisrs$meals, xpop = pop_totals, datatype = "totals", pi = NULL, N = dim(apipop)[1], pi2= NULL, var_est = TRUE, var_method = "HB", B = 1000)
re_srs_totals
```
Note that the three estimates agree completely with each other, as we should expect.
