...
\hat{\boldsymbol{\beta}_{s}} = \underset{\boldsymbol{\beta}}{\text{arg min}} 
                              ...
                              \right \}
$$
Observe that the first half of the equation is the original GREG formula. The second half of the equation is the aforementioned "elastic net," which is the penalty calculation.The elastic net itself is split again into two halves. The first half of the elastic net, $\alpha\sum_{j=1}^p \left|\beta_{j} \right|$, is known as the LASSO penalty, which shrinks the amount of unnecessary variables. The second half of the elastic net, $(1-\alpha)\sum_{j=1}^p \beta^2_{j}$, is the Ridge Regression penalty, which is good for multicollinearity. The alpha value decides the relative amount weight to assign to each type of penalty. 

better than brute force choosing subsets because too tedious. penalization section chooses the "important" betas for us

With the addition of this elastic net condition, the Elastic Net Regression estimator has a better variability than the GREG. The presence of too many variables from available auxiliary data can add extra variability to GREG, whereas the elastic net condition shrinks the coefficients towards zero. 
...
```
add more variables to elastic to show discarded variables
add elastic logistic
add std to greg and elastic
update function

paragraph: observe discarded variables / talk about changes in std
