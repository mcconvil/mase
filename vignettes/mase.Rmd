...
$$
# HOW DO YOU PUT BETA BELOW ARGMIN????
\\
\hat{\boldsymbol{\beta}_{s}} = arg min\big\{\sum_{i \in s}
                              \frac{(yi-\boldsymbol{x}_{i}^T \boldsymbol{\beta})^2}
                              {\pi_{i}\sigma^2_{i}} +
                              \lambda[\alpha\sum_{j=1}^p|\beta_{j}| + 
                              (1-\alpha)\sum_{j=1}^p \beta^2_{j}
                              \big\}
$$
...
gregElastic_lin_srs <- gregElasticNet(y = apisrs$awards, 
                                      xsample = apisrs[c("col.grad", "sch.wide")],
                                      xpop = apipop[c("col.grad", "sch.wide")], 
                                      pi = apisrs$pw^(-1), model = "linear")
gregElastic_lin_srs
...
gregElastic_lin_strat <- gregElasticNet(y = apistrat$awards, 
                                      xsample = apistrat[c("col.grad", "sch.wide")],
                                      xpop = apipop[c("col.grad", "sch.wide")],
                                      pi = apistrat$pw^(-1), model = "linear")
