...
A common goal in survey sampling is to estimate finite population parameters, such as the total
$$
t_y = \sum_{i \in U} y_i
$$
or the mean $\mu_y = N^{-1}t_y$ for some attribute of interest, $y$.  When $y$ is a binary, categorical measure, then $\mu_y$ represents a proportion. 
...
where for observation $i$, $\hat{y}_i$ is the predicted value of $y$ and $\pi_i = P(i \in s)$, the inclusion probability [@cas76; @sar92]. As the model for $\hat{y}_i$ varies, we get different model-assisted estimators.  The estimators in mase along with the assisting model, necessary auxiliary data, and a reference, are given in the following table.
...
If there is a categorical, auxiliary variable that is predictive of the study variable, then a post-stratified estimator could be more efficient than the Horvitz-Thompson estimator.  

For example, there is a binary,categorical variable, `awards`, which specifies whether or not a school is eligible for an awards program.   From the graph below, it appears that eligible schools have, on average, a higher api score.
...
To add the awards information, we need to include the sample values (`xsample`) and the population data (`xpop`) can be given in one of three ways as shown in the following R chunk.
...
All survey estimators listed above (ie. Horvitz-Thompson, Post-Stratified, Ratio) can be rewritten in the form of the generalized regression estimator:
$$
\hat{\mu_{y}} = \frac{1}{N} \left(\sum_{i \in s}\frac{y_{i}-\hat{m}(x_{i})}{\pi_{i}} 
                + \sum_{i \in U}\hat{m}(x_{i}) \right)
$$
