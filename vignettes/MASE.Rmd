...
$$
Mention linear vs. logistic regression? How in depth should I go?
...
greg_srs <- greg(y = apisrs$api00, 
                 xsample = data.frame(apisrs$col.grad, apisrs$awards),
                 xpop = data.frame(apipop$col.grad, apipop$awards), 
                 pi = apisrs$pw^(-1), model = "linear", N = 6194)
